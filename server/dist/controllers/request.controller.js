...
export const createBloodRequest = async (req, res) => {
    try {
        ...
        if (!Object.values(RequestUrgency).includes(urgency)) {
            res.status(400).json({
                success: false,
...
export const getBloodRequests = async (req, res) => {
    try {
        ...
        if (urgency && Object.values(RequestUrgency).includes(urgency)) {
        ...
        try {
            const transactionResult = await prisma.$transaction(async (tx) => {
                ...
                if (available < bloodRequest.units_requested) {
                    const shortageError = new Error('INSUFFICIENT_STOCK');
